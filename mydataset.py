...
def load_data(filedir, filename, dtype=np.float32, wo=False):
    filepath = os.path.join(filedir, filename + '.npy')
...
class PointCloudDataset(Dataset):
    ...
    def get_data(self, cur_sets):
        for s in tqdm(cur_sets, desc='Loading data'):
            pcl = load_data(filedir=self.data_dir, filename='%s/coord' % s, dtype=np.float32)[:, :3]

            if os.path.exists(os.path.join(self.data_dir, s + '/normal.npy')):
                nor = load_data(filedir=self.data_dir, filename=s + '/normal', dtype=np.float32)
            else:
